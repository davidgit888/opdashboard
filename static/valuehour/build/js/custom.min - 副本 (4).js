function toggle_local() {
	res = localStorage.getItem("sidebar");
	if (res && res == 'md') {
		localStorage.setItem("sidebar", "sm");
	} else if (res && res == 'sm') {
		localStorage.setItem("sidebar", "md");
	} else {
		localStorage.setItem("sidebar", "sm")
	}
};
...
function init_sidebar() {
	localStorage.getItem("sidebar") == "md" || localStorage.getItem("sidebar") == "sm" ? $(document.body).attr({
		'class': 'nav-' + localStorage.getItem("sidebar")
	}) : void(0);
	var a = function() {
	...
	$SIDEBAR_MENU.find("a").on("click", function(b) {
		...
		toggle_local(), $BODY.hasClass("nav-md") ? ($SIDEBAR_MENU.find("li.active ul").hide(), $SIDEBAR_MENU.find("li.active").addClass("active-sm").removeClass("active")) : ($SIDEBAR_MENU.find("li.active-sm ul").show(), $SIDEBAR_MENU.find("li.active-sm").addClass("active").removeClass("active-sm")), $BODY.toggleClass("nav-md nav-sm"), a()
	}), $SIDEBAR_MENU.find('a[href="' + CURRENT_URL + '"]').parent("li").addClass("current-page"), agent = $SIDEBAR_MENU.find("a").filter(function() {
		return this.href == CURRENT_URL
	}).parent("li").addClass("current-page"), agent = localStorage.getItem("sidebar") !== 'sm' ? agent.parents("ul").slideDown(function() {
		a()
	}).parent().addClass("active") : void(0), $(window).smartresize(function() {
		a()
